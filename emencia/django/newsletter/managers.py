...
""" ADDED BulkInsert MAnager based on http://djangosnippets.org/snippets/446/ """
...
from django.db.models.fields import AutoField, DateTimeField, DateField, TimeField, FieldDoesNotExist, BooleanField
...
class BulkInsert(models.Manager):
    ...
    def _recover_pks(self, _self_ref=False):
        ...
        # recovering pks based on huge selects packeted by 100 can be very slow for thousands of insert
        # OPTIMIZE by selecting pk filtered upon creation_date
        dt_nowfields = [f for f in self.model._meta.fields if isinstance(f, DateTimeField) and (f.default == datetime.datetime.now or f.auto_now or f.auto_now_add)]
        if len(dt_nowfields)>0:
            first_dt_nowfields = dt_nowfields[0] 
            sql = "SELECT %s FROM %s WHERE " % \
                        (','.join(["%s.%s" % (qn(table), qn(f.column)) for f in recovery_fields]), qn(table)) + \
                        "%s = '%s'" % (qn(first_dt_nowfields.column), self.now.strftime('%Y-%m-%d %H:%M:%S')) + \
                        " ORDER BY %s" % qn(primary.column)
            ...
            # optimization path ended 
            return

        

        # fallback to 100 packeted selects to recover if no optimization could be done 
        recover_limit = 100
        start = 0
...
class MySQL_BulkInsert(BulkInsertBackend):
    def write_temp_file(self, fields, queue):
        ...
        for kwargs in queue.values():
            ...
            for f in fields:
                ...
                # because LOAD DATA INFILE generate warnings upon BooleanField default value
                # force its value to 1 or 0
    ...
    def insert_from_file(self, table, columns, autoclobber=False):
        ...
        # added LOCAL as said in MySQL manual http://dev.mysql.com/doc/refman/5.1/en/load-data.html 
        # history of the prblem here : https://bugs.launchpad.net/ubuntu/+source/mysql-dfsg-5.0/+bug/244406
        query = "LOAD DATA LOCAL INFILE '%s' %s INTO TABLE %s (%s)" % \
...
class ContactManager(BulkInsert):
